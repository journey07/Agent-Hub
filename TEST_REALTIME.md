...
`testRealtimeInsert()` 함수는 다음 조건을 만족하는 페이지에서만 사용 가능합니다:
...
❌ **사용 불가능한 페이지:**
- `/login` (로그인 페이지) - 인증되지 않아서 작동하지 않음
- **로그인하지 않은 상태** - 함수가 정의되지 않음

#### 테스트 방법

1. **대시보드 페이지 열기** (로그인 상태, 위의 사용 가능한 페이지 중 하나)
2. **브라우저 개발자 도구 열기** (F12)
3. **Console 탭**에서 다음 실행:

```javascript
// 먼저 함수가 정의되어 있는지 확인
typeof testRealtimeInsert  // "function"이 나와야 함

// 인증 상태 확인
checkAuthStatus()  // 세션이 있어야 함
